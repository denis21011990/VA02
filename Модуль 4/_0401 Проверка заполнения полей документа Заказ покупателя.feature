...
* Заполнение и проверка Партнера
...
* Заполнение и проверка Соглашения
	...
	И в таблице "List" я перехожу к строке:
		| 'Код' |
		| '2'   |
	И в таблице "List" я активизирую поле "Код"
...
* Проверка перезаполнения соглашения при изменении партнера с одним соглашением
	Когда открылось окно 'Заказ покупателя (создание) *'
	И я нажимаю кнопку выбора у поля "Партнер"
	Тогда открылось окно 'Партнеры'
	И в таблице "List" я перехожу к строке:
...
* Проверка перезаполнения соглашения при изменении партнера с двумя и более соглашениями
	Когда открылось окно 'Заказ покупателя (создание) *'
	И я нажимаю кнопку выбора у поля "Партнер"
	Тогда открылось окно 'Партнеры'
	И в таблице "List" я перехожу к строке:
